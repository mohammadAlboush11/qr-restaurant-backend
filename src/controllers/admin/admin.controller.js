...
class AdminController {
  ...
  async getDashboardStats(req, res) {
    try {
      ...
      // Korrigierte Revenue by Month Query
...
// Korrigierte Subscriptions by Plan Query  
  const subscriptionsByPlan = await sequelize.query(`
    SELECT 
      p.name as plan_name,
      COUNT(s.id) as count
    FROM plans p
    LEFT JOIN subscriptions s ON s.plan_id = p.id AND s.status = :status
      ...
      if (ActivityLog) {
        const logs = await ActivityLog.findAll({
          include: [
            ...
            {
              model: Restaurant,
              as: 'restaurant',  // KORRIGIERT: Verwende den richtigen Alias
